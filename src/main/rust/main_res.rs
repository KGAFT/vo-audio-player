...
impl DSDReader for DSFReader {
    fn open(&mut self, format: &mut DSDFormat) -> io::Result<()> {
        ...
        // seek to fmt chunk (original C++ did seekg(size - 12, cur))
        // we follow same approach: skip size - 12 bytes
        // (C++ read size then seek size - 12 because 12 bytes were already read)
        // But safer: read chunk until "fmt " is found. We'll mimic the C++ behaviour:
        ...
        if channels != 2 {
            // we'll still set value, but caller expects stereo
        }
        ...
        // seek to end of fmt chunk like original: seek(size - 48, cur)
    ...
    fn read(&mut self, data: &mut [&mut [u8]], bytes_per_channel: usize) -> io::Result<usize> {
        ...
        while want > 0 {
            if self.pos == self.filled {
                ...
                // the C++ computed _filled = gcount() / _ch
...
fn main() -> io::Result<()> {
    let filename = "/mnt/files2/Music/Dire Straits - Brothers In Arms - 1985(2021),(USA),DSF(tracks),(OC-9XMLf+MF(115)avb+Pio)/A1 So Far Away.dsf";
    ...
    unsafe {
        ...
        if (alsa::snd_pcm_hw_params_malloc)(&mut hw_params) < 0 {
            ...
            if !playback_handle.is_null() { alsa::snd_pcm_close(playback_handle); }
            return Ok(());
        }
